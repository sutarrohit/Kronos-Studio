    ...
    origNodes.forEach((origNode, i) => {
        ...
        if (tagName === "text" || tagName === "tspan") {
            ...
            if (
                ...
                (!computedFill || computedFill === "rgb(0, 0, 0)")
            ) {
...
function appendLegendToSVG(
    ...
    const estimatedWidths = collected.map(
        ({ label }) => ICON_LEN + ICON_TEXT_GAP + label.length * 7
    );
    const totalWidth =
        estimatedWidths.reduce((a, b) => a + b, 0) +
        ITEM_GAP * (collected.length - 1);
...
export async function downloadBatchAsZip(
    ...
    for (let i = 0; i < batchResults.length; i++) {
        ...
        const prefix = buildFilenamePrefix(res);
